...
class SDIM(BaseModel):
    def __init__(self,
        ...
        super(SDIM, self).__init__([], dnn_feature_columns, l2_reg_linear=0, l2_reg_embedding=l2_reg_embedding,
        ...
        self.n_hash = n_hash
        self.random_rotation = nn.Parameter(torch.randn(att_emb_dim, self.n_hash // self.hash_bits, self.hash_bits) * 0.02, requires_grad=False)
        self.signature_generator = nn.Parameter(torch.tensor([2. ** i for i in range(self.hash_bits)]), requires_grad=False)
        self.to(device)

    def attention_lsh(self, query, key, length):
        def lsh(input):
            input_hash = torch.einsum('abc,cde->abde', input, self.random_rotation)  # [B T d] [d n tao] -> [B T n tao]
            input_hash = torch.relu(torch.sign(input_hash))
            signature = torch.matmul(input_hash, self.signature_generator.unsqueeze(-1)).squeeze(-1)        # [B T n tao] [tao 1] -> [B T n]
            return signature

        query_signature = lsh(query)
        key_signature = lsh(key)
        collision = (key_signature - query_signature).permute(0, 2, 1)    # [B n T]
        collision = collision == 0.

        max_length = key.shape[1]
        mask = torch.arange(max_length, device=self.device).repeat(len(length), 1)  # [T]
        mask_length = max_length - length                                   # [B T]
        mask = (mask >= mask_length.view(-1, 1)).unsqueeze(dim=1)
        # [B 1 T]
        masked_collision = torch.logical_and(mask, collision).float()                  # [B n T]
        attention_signature = torch.bmm(masked_collision, key)                         # [B n T] [B T d] -> [B n d]
        attention_signature = F.normalize(attention_signature, 2, dim=1)
        attention = attention_signature.mean(1, keepdim=True)                          # [B 1 d]
        return attention

    def attention_lsh_2(self, query, key, length):
        def lsh(input):
            input_hash = torch.einsum('abc,cde->abde', input, self.random_rotation)  # [B T d] [d n tao] -> [B T n tao]
            input_hash = torch.relu(torch.sign(input_hash))
            signature = torch.matmul(input_hash, self.signature_generator.unsqueeze(-1)).squeeze(-1)        # [B T n tao] [tao 1] -> [B T n]
        ...
        query_signature = lsh(query)
        key_signature = lsh(key)
        collision = (key_signature - query_signature).permute(2, 0, 1)    # [n B T]
        collision = collision == 0.
        max_length = key.shape[1]
        mask = torch.arange(max_length, device=self.device).repeat(len(length), 1)  # [T]
        mask_length = max_length - length  # [B T]
        mask = (mask >= mask_length.view(-1, 1))
        collide_mask = (collision * mask.unsqueeze(0)).float()  # both hash collision mask and sequence mask
        ...
        attn_out = F.embedding_bag(collide_index, key.reshape(-1, query.shape[-1]),
                                   offsets, mode='sum', include_last_offset=True)  # (num_hashes x B) x d
        attn_out = attn_out.reshape(-1, query.shape[0], query.shape[-1]).mean(dim=0)  # B x d
        return attn_out.unsqueeze(1)
    ...
    def forward(self, X):
        ...
        # long
        long_hist = self.attention_lsh(query_emb, long_keys_emb, long_keys_length)
