...
class AttentionSequencePoolingLayer(nn.Module):
    ...
    def forward(self, query, keys, keys_length, mask=None):
        ...
        if self.supports_masking:
            ...
            keys_length = max_length - keys_length  # update
            keys_masks = keys_masks >= keys_length.view(-1, 1)  # 0, 1 mask  # update
...
class MultiheadAttention(nn.Module):
    ...
    def forward(self, query, key, value, keys_length=None):
        bsz = query.shape[0]
        Q = self.w_q(query)
        K = self.w_k(key)
        V = self.w_v(value)
        # [B,L,D] -> [B,L,N,d] -> [B,N,L,d]
        ...
        if keys_length is not None:
            B, T, E = key.size()
            keys_masks = torch.arange(T, device=key.device, dtype=key.dtype).repeat(B, 1)  # [B, T]
            keys_length = T - keys_length  # update
            keys_masks = keys_masks >= keys_length.view(-1, 1)  # 0, 1 mask  # update
            keys_masks = keys_masks.view(bsz, 1, 1, -1).expand(-1, self.n_heads, -1, -1)
            attention = attention.masked_fill(keys_masks == 0, -1e10)
