...
from deepctr_torch.models.sdim import SDIM
...
if __name__ == "__main__":
    ...
    [user_length, item_length, cate_length, seller_length] = np.load('../data/alipay/alipay_len_data.npy',
                                                                     allow_pickle=True)
    if max_sequence_len == 16:
        train_feature_dict = np.load('../data/alipay/alipay_16_train.npy', allow_pickle=True).item()
        test_feature_dict = np.load('../data/alipay/alipay_16_test.npy', allow_pickle=True).item()
        [user_length, item_length, cate_length, seller_length] = np.load('../data/alipay/alipay_16_len_data.npy',
                                                                         allow_pickle=True)
    ...
    print(f'hash_bits={hash_bits}, n_hash={n_hash}')

    model = SDIM(feature_columns, behavior_feature_list, device=device,
                 att_weight_normalization=True, dnn_use_bn=True,
                 short_long_length=short_long_length, hash_bits=hash_bits, n_hash=n_hash)

    model.compile('adam', 'binary_crossentropy',
                  metrics=['auc', 'logloss'])
    history = model.fit(train_x, train_y, batch_size=256, epochs=1, verbose=2,
